...
def create_game():
    ...
    if 'name' not in data:
        return jsonify({'error': '必須包含遊戲名稱'}), 400
...
def update_game_prize(game_id, position):
    ...
    data = request.get_json() or {}
    if 'name' not in data:
        return jsonify({'error': '必須包含獎項名稱'}), 400
